...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if "VL-3B-Instruct" in pretrained_model_name_or_path:
            print("------------注意----------\n-----------所有的动作头参数将重新初始化")
            if getattr(model, "query_action", None) is not None:
            ...
            if getattr(model, "query_multihead_attn", None) is not None:
                ...
                for name, p in attn_mod.named_parameters():
                    if "weight" in name:
                        ...
                        p.data.zero_() 
            def init_action_former_head(action_head):
                for name, module in action_head.named_modules():
                    ...
                    # 对于LayerNorm，bias 置零，weight 置 1
                    elif isinstance(module, nn.LayerNorm):
                        if module.elementwise_affine:
                            nn.init.ones_(module.weight)
                            nn.init.zeros_(module.bias)

            # 假设你的模型里有 self.query_multihead_multi_attn = ActionFormerHead(...)
            if getattr(model, "query_multihead_multi_attn", None) is not None:
                init_action_former_head(model.query_multihead_multi_attn)
